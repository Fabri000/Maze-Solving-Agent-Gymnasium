...
class MazeEnv(gym.Env):
    ...
    def __init__(self, maze_map,start_pos,goal_pos, render_mode = "human"):
        ...
        if self.render_mode == "human":
            self.maze_view = SimpleMazeView(self.maze_map,self._start_pos,self._goal_pos,(len(maze_map),len(maze_map[1])))
    ...
    def _find_best_next_cell(self,agent_pos):
        ...
        for dir in MazeEnv.ACTIONS:
            next_pos = ((agent_pos[0] + MazeEnv.ACTIONS[dir][0]) % self.maze_shape[0] , (agent_pos[1] + MazeEnv.ACTIONS[dir][1]) % self.maze_shape[1] )#change
