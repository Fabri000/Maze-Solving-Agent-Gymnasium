...
agent = DDQNAgent(env,learning_rate=learning_rate,starting_epsilon=starting_epsilon,final_epsilon=final_epsilon,epsilon_decay=epsilon_decay,discount_factor=discount_factor,eta=eta,batch_size=batch_size,memory_size=100000,target_update_frequency=1,device=device)

logger = init_logger("Agent_log","logs/tor_variable_ddqn_logs")
logger.info(f"Training starting on variable mazes with dimension variable between {(29,29)} and {maze_max_shape}")
